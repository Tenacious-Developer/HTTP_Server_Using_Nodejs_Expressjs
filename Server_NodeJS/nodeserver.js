...
const server = http.createServer(function listener(request, response) {
    ...
    if(request.url == '/home') {
        ...
        console.log(request.method); // from browser we got only GET Request
        
        response.end("completed"); // accept only string argument
...
// This process is on going continue untill we stop the server using ctrl+c or runtime exception.
// 127.0.0.1 is a special-purpose IPv4 address that's reserved for the local host or computer. 
// Any traffic sent to this address is routed back to the same computer.
// It allows the machine to connect to and communicate with itself.
// It's reserved for loop back testing.



// send function doesn't exist in nodejs http module 